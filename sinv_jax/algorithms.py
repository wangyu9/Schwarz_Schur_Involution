"""
Pure-Python / NumPy index helpers. These don't touch the GPU, so the Triton
port is a faithful copy of sinv/algorithms.py.
"""
...
def test_inverse_permutation(wh=[5, 5]):
    ...
    UB = matrix_interior_list(wh[0], wh[1])
    print(B + UB)
    ordering = inverse_permutation(B + UB)
    print(ordering)
    tmp = inverse_permutation(ordering)
    print(tmp)
